...
import com.suven.framework.upload.service.impl.FileParseServiceImpl;
...
class SaaSFileParseServiceTest extends AbstractUnitTest {
    ...
    private List<FileFieldMapping> fieldMappings;

    @BeforeEach
    void setUp() {
        logTestStart("FileParseServiceTest");
        // 初始化字段映射配置
    ...
    void testConvertDataRow() {
        ...
        List<FileFieldMapping> mappings = createFieldMappings();
    ...
    void testValidateDataRow() {
        ...
        // given
        List<FileFieldMapping> mappings = createFieldMappingsWithRequired();
    ...
    void testParseCsvFile() throws IOException {
        ...
        // when
        FileParseResultDto result = fileParseService.parse(inputStream, "test.csv", "csv");
    ...
    void testParseWithMapping() throws IOException {
        ...
        // when
        FileParseResultDto result = fileParseService.parseWithMapping(
                inputStream,
                "test.csv",
                "csv",
                fieldMappings
     ...
     * 创建字段映射配置
     */
    private List<FileFieldMapping> createFieldMappings() {
        List<FileFieldMapping> mappings = new ArrayList<>();
     ...
     * 创建带必填校验的字段映射
     */
    private List<FileFieldMapping> createFieldMappingsWithRequired() {
        List<FileFieldMapping> mappings = new ArrayList<>();

        FileFieldMapping nameMapping = createFieldMapping("name", "姓名", 1);
        nameMapping.setIsRequired(1);
     ...
     * 创建单个字段映射
     */
    private FileFieldMapping createFieldMapping(String englishName, String chineseName, int sortOrder) {
        return FileFieldMapping.builder()
                .fieldEnglishName(englishName)
                .fieldChineseName(chineseName)
                .sortOrder(sortOrder)
