...
public class YouBianCodeUtil {
	...
	public static synchronized String getNextYouBianCode(String code) {
		return  getNextYouBianCode(code,numLength);
	}
	public static synchronized String getNextCode(String code) {
		return  getNextYouBianCode(code,THREE_LENGTH);
	}

	protected static synchronized String getNextYouBianCode(String code, int numLength) {
		String newcode = "";
		if (code == null || code =="") {
			String zimu = "A";
			String num = getStrNum(1,numLength);
			newcode = zimu + num;
		} else {
...
//			org.jeecgframework.core.util.LogUtil.info(after_code_num);
			...
			if (after_code_num == getMaxNumByLength(numLength)) {
				nextNum = getNextStrNum(0,numLength);
			} else {
				nextNum = getNextStrNum(after_code_num,numLength);
			}
	 ...
	 * @return
	 */
	public static synchronized String getCode(String parentCode,String localCode) {
		return getSubYouBianCode(parentCode,localCode,THREE_LENGTH);
	}
	/**
	 * 根据父亲code,获取下级的下一个code
	 *
	...
	public static synchronized String getSubYouBianCode(String parentCode,String localCode) {
		return getSubYouBianCode(parentCode,localCode,numLength);
	}
	protected static synchronized String getSubYouBianCode(String parentCode,String localCode,int numLength) {
		if(localCode!=null && localCode!=""){

//			return parentCode + getNextYouBianCode(localCode);
			return getNextYouBianCode(localCode,numLength);

		}else{
			parentCode = parentCode + "A"+ getNextStrNum(0,numLength);
		}
		return parentCode;
	}
	...
	public static String[] cutCode(String code){
		return cutYouBianCode(code,THREE_LENGTH);
	}
	protected static String[] cutYouBianCode(String code, int numLength ){
