...
public class SysPermissionDataRuleResponseVo  extends BaseByTimeEntity implements Serializable {
 		...
 		@ExcelProperty(value = "  `status` int DEFAULT NULL COMMENT '删除状态(1-正常,0-已删除)',")
 		private int status;
    ...
    public static SysPermissionDataRuleResponseVo build(){
        return new SysPermissionDataRuleResponseVo();
 		...
 		public SysPermissionDataRuleResponseVo toStatus( int status){
 		 		this.status = status ; 
 		 		 return this ;
 		}
