...
/**
 * Title: RoleHandlerInterceptor.java
 * @author Joven.wang
 * @version V1.0
 ...
 *    修改后版本:     修改人：  修改日期:     修改内容:
 * </pre>
 * Description: (说明) http 接口拦截器,主要是校验接口是否触角色是否包括接口业务,排序值为4;
 */


public class RoleHandlerInterceptor extends AbstractHandlerInterceptorAdapter implements IHandlerInterceptor {

    private static Logger log = LoggerFactory.getLogger(RoleHandlerInterceptor.class);

    public RoleHandlerInterceptor(ApplicationContext application, InterceptorConfigSetting handlerSetting) {
    ...
    @Override
    public int getOrder() {
        return HandlerInterceptorEnum.HANDLER_ORDER_ROLE.getIndex();
    }
    ...
    public String handlerValidatorBeanType() {
        return HandlerInterceptorEnum.HANDLER_ORDER_ROLE.getValue();
    }
