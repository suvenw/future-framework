 ...
 *    修改后版本:     修改人：  修改日期:     修改内容:
 * </pre>
 * Description: (说明) http 接口统一请求头信息方法实现转换逻辑实现业务类;
 */

public class RequestHeaderHandlerArgumentResolver extends AbstractHandlerArgumentResolver<HttpHeaderRequest> {
    ...
    protected Object onResolve(MethodParameter parameter,
        ...
        **/
        HttpServletRequest request = webRequest.getNativeRequest(HttpServletRequest.class);
        HttpServletResponse response = webRequest.getNativeResponse(HttpServletResponse.class);
        HttpRequestTypeEnum httpRequestTypeEnum = HttpRequestTypeEnum.TYPE_HEADER;
    ...
    public Object parserDate(HttpServletRequest webRequest, HttpServletResponse response, Class<?> clazz, HttpRequestTypeEnum httpRequestTypeEnum) {
        return this.parseHeaderData(webRequest,response,clazz,httpRequestTypeEnum);
    }
}