...
@Component("redisHandlerValidator")
public class RedisHandlerValidator extends ValidatorCache<Map<String, VersionHandlerVo>>  implements HandlerValidator {


    private Logger logger = LoggerFactory.getLogger(RedisHandlerValidator.class);
...
//                object = ReflectionUtilTool.invokeBeanMethod( validatorService, GlobalConfigConstants.VERSION_HANDLER_METHOD_NAME,null);
    ...
    private String cacheUpdateVersionKey() {
        ...
        String updateKey = TopStringUtils.toIntegerBuilder(
                message.getPlatform(),message.getChannel(),1);
        return updateKey;
    }
    ...
    private String cacheNewVersionKey() {
        HttpRequestPostMessage message = ParameterMessage.getRequestMessage();
        String updateKey = TopStringUtils.toIntegerBuilder(
                message.getPlatform(),message.getChannel(),0);
        return updateKey;
    }
